...
function AdminDashboardRed() {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      ...
      console.log('Fetching RED jobs');
      
      // Fetch jobs data only
      let jobsData = [];
      try {
        const jobsResponse = await axios.get(`${API_URL}/jobs_red.php`, { headers: getAuthHeader() });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingJob) {
        ...
        // await axios.put(`${API_URL}/jobs.php?id=${editingJob.id}`, formData, { headers: getAuthHeader() });
        alert('Edit not implemented for Red yet'); // Placeholder
      } else {
        // Create Job (POST request)
        await axios.post(`${API_URL}/jobs_red.php`, formData, { headers: getAuthHeader() }); // Use _red endpoint
      }
      handleCloseDialog();
  ...
  const handleDelete = async (jobId) => {
    if (window.confirm('Are you sure you want to delete this job?')) {
      try {
        ...
        await axios.delete(`${API_URL}/job_delete_red.php?id=${jobId}`, { headers: getAuthHeader() }); // Use _red endpoint
        fetchJobs(); // Refresh job list
      } catch (error) {
...
export default AdminDashboardRed; 